...
public class MapItemWithinTask extends AsyncTask<String, Void, ContentValues[]>
{
    ...
    protected ContentValues[] doInBackground(String... tables)
    {
        ...
        for (int i=0; i<tables.length; i++)
        {
            if (tables[i] != null)
            {
                ContentValues[] contentValues = database.findMapsWithBounds(tables[i], TopoIndexDatabaseAdapter.QUERY_MAPS_LISTENTRY, mapScale, (mapScale != TopoIndexDatabaseAdapter.MapScale.SCALE_ANY), item);
